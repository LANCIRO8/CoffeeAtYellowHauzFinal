      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl sm:rounded-3xl bg-white shadow-2xl border border-stone-200 animate-in zoom-in-95 duration-200">
        ...
        <div className="bg-gradient-to-br from-amber-500 via-amber-400 to-yellow-500 px-4 py-3 sm:px-5 sm:py-4 text-stone-950 relative">
          <button
            onClick={onClose}
            title="Close modal"
            className="absolute top-2.5 right-2.5 sm:top-3 sm:right-3 rounded-full bg-black/10 hover:bg-black/20 p-1 sm:p-1.5 text-stone-900 transition cursor-pointer"
          >
            <X className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
          </button>

          <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-3 pr-6 sm:pr-7">
            <div className="flex items-center gap-2 sm:gap-2.5">
              <div className="grid h-7 w-7 sm:h-9 sm:w-9 place-items-center rounded-lg sm:rounded-xl bg-stone-950 text-amber-400 shadow-md shrink-0">
                {isLiveInHouse ? (
                  <Coffee className="h-4 w-4 sm:h-5 sm:w-5 stroke-[2.2]" />
                ) : (
                  <Calendar className="h-4 w-4 sm:h-5 sm:w-5 stroke-[2.2]" />
                )}
              </div>
              <h2 className="font-display text-sm sm:text-xl font-black tracking-tight">
                {isLiveInHouse ? 'Live Order Placed!' : 'Advance Booking Confirmed!'}
              </h2>
            </div>

            <div className="rounded-lg sm:rounded-xl bg-stone-950/10 px-2 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-xs font-bold text-stone-900 border border-stone-950/15">
              <span>Order Ref: </span>
        ...
        {/* Content Body */}
        <div className="p-3.5 sm:p-6 max-h-[72vh] sm:max-h-[70vh] overflow-y-auto space-y-3 sm:space-y-5">
          {/* External Advance Booking Details (for advance bookings only) */}
          {!isLiveInHouse && (order.advanceBooking || order.scheduledFor) && (
            <div className="rounded-xl sm:rounded-2xl border border-amber-300 bg-amber-50/90 p-3 sm:p-4 space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 sm:gap-2 text-amber-950 font-extrabold text-xs sm:text-sm">
                  <Globe className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-amber-700" />
                  <span>External Advance Booking</span>
                </div>
                <span className="rounded-full bg-amber-200 text-amber-950 font-mono text-[10px] sm:text-xs font-black px-2 py-0.5">
                  SCHEDULED
                </span>
              </div>
              {order.advanceBooking ? (
                <div className="grid grid-cols-2 gap-2 text-[11px] sm:text-xs bg-white/70 rounded-xl p-2 sm:p-2.5 border border-amber-200">
                  <div>
                    <span className="text-stone-400 block text-[9px] sm:text-[10px] uppercase font-bold">Target Date</span>
                    <span className="font-bold text-stone-800">{order.advanceBooking.bookingDate}</span>
                  </div>
                  <div>
                    <span className="text-stone-400 block text-[9px] sm:text-[10px] uppercase font-bold">Arrival Time</span>
                    <span className="font-bold text-stone-800">{order.advanceBooking.arrivalTime}</span>
                  </div>
                  {order.advanceBooking.partySize && (
                    <div>
                      <span className="text-stone-400 block text-[9px] sm:text-[10px] uppercase font-bold">Party Size</span>
                      <span className="font-bold text-stone-800">{order.advanceBooking.partySize} Guests</span>
                    </div>
                  )}
                  {order.advanceBooking.seatingPreference && (
                    <div>
                      <span className="text-stone-400 block text-[9px] sm:text-[10px] uppercase font-bold">Seating Area</span>
          ...
          {/* Progress Timeline */}
          <div className="rounded-xl sm:rounded-2xl bg-stone-50 p-2.5 sm:p-4 border border-stone-200">
            <div className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-stone-500 mb-2 sm:mb-3">
              Order Workflow
            </div>
            <div className="grid grid-cols-3 gap-1.5 sm:gap-2 text-center text-xs">
              <div className="flex flex-col items-center gap-1 sm:gap-1.5">
                <div className="grid h-6 w-6 sm:h-7 sm:w-7 place-items-center rounded-full bg-emerald-500 text-white shadow-xs text-[10px] sm:text-xs font-black">
                  ✓
                </div>
                <span className="font-bold text-stone-800 text-[10px] sm:text-[11px]">Submitted</span>
                <span className="text-[9px] sm:text-[10px] text-emerald-600 font-semibold">Done</span>
              </div>

              <div className="flex flex-col items-center gap-1 sm:gap-1.5">
                <div className="grid h-6 w-6 sm:h-7 sm:w-7 place-items-center rounded-full bg-amber-500 text-stone-950 shadow-xs text-[10px] sm:text-xs font-black ring-2 sm:ring-4 ring-amber-200/70">
                  2
                </div>
                <span className="font-bold text-stone-900 text-[10px] sm:text-[11px]">
                  {isLiveInHouse ? 'Kitchen Queue' : 'Booking Logged'}
                </span>
                <span className="text-[9px] sm:text-[10px] text-amber-700 font-bold">In Queue...</span>
              </div>

              <div className="flex flex-col items-center gap-1 sm:gap-1.5">
                <div className="grid h-6 w-6 sm:h-7 sm:w-7 place-items-center rounded-full bg-stone-200 text-stone-500 text-[10px] sm:text-xs font-black">
                  3
                </div>
                <span className="font-medium text-stone-500 text-[10px] sm:text-[11px]">
                  {isLiveInHouse ? 'Serve to Table' : 'Arrive & Enjoy'}
                </span>
                <span className="text-[9px] sm:text-[10px] text-stone-400">Upcoming</span>
              </div>
            </div>
          </div>
        ...
        <div className="p-3 sm:p-4 bg-stone-50 border-t border-stone-200 flex flex-col sm:flex-row gap-2">
          {onTrackOrder ? (
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 rounded-xl bg-amber-500 hover:bg-amber-400 px-3 py-2.5 sm:px-4 sm:py-3 text-[11px] sm:text-xs font-extrabold text-stone-950 shadow-xs transition cursor-pointer"
            >
